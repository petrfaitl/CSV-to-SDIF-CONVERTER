<script lang="ts" setup>
  ...
  const navItems = [
    {
      id: 0,
      title: "Swim Night Registration",
      link: "/",
    },
    {
      id: 1,
      title: "Squad Details",
      link: "http://www.lvwasc.co.nz/squad-details/",
      type: 'external',
    },
    {
      id: 2,
      title: "About",
      link: "http://www.lvwasc.co.nz/about-us/",
      type: 'external',
    },
    {
      id: 3,
      title: "News",
      link: "http://www.lvwasc.co.nz/news/",
      type: 'external',
    }
  ];
...
<template>
  <div class="relative w-full bg-white text-sm h-16">
      ...
      >
        ...
        <div class="flex flex-wrap relative">
          ...
          <div class="px-6 h-16 w-full border-b flex items-center">
            <NuxtLink to="/">
              <img src="/images/lvwa_logo.jpg" alt="LVW Aquatics logo" class="object-contain h-16">
            </NuxtLink>
          ...
          <ul class="flex flex-col gap-4 pt-6 px-6">
            <li v-for="item in navItems" :key="item.id">
              <NuxtLink :to="`${item.link}`" class="uppercase " :target="item?.type ==='external'?'_blank':'_self'">
                {{ item.title }}
              </NuxtLink>
            </li>
          </ul>
    ...
    <div class="fixed inset-x-0 top-0 h-16 w-full bg-white z-20">
      <div class="flex flex-no-wrap px-4 md:px-12 h-full border-b items-center gap-8">
        <!--            Hamburger Button-->
        ...
        <ul class="lg:flex gap-8 hidden" @click.passive="closeMobileMenu" v-if="!mobileMenuIsOpen">
          <li v-for="item in navItems" :key="item.id">
            <NuxtLink :to="`${item.link}`"
                      :target="item?.type ==='external'?'_blank':'_self'"
                      class="uppercase font-semibold">{{ item.title }}
            </NuxtLink>
          </li>
        </ul>
        <div class="ml-auto">
